...
class Light:
    ...
    def on(self) -> None:
        print(f"Luz da {self.name} do {self.room_name} está ON")

    def off(self) -> None:
        print(f"Luz da {self.name} do {self.room_name} está OFF")

    def change_color(self, color: str) -> None:
        self.color = color
        print(
            f"Luz da {self.name} do {self.room_name} está na cor {self.color}"
        )


class ICommand(ABC):
    '''IOcomand - Interface de comando'''
    @abstractmethod
...
class LightOnCommand(ICommand):
    '''ConcreteConcrete - Comando Concreto'''
...
class LightChangeColor(ICommand):
    ...
    def __init__(self, light: Light, color: str) -> None:
        self.light = light
        self.color = color
        self._old_collor = self.light.color
    ...
    def undo(self) -> None:
        self.light.change_color(self._old_collor)
...
class RemoteController:
    ...
    def button_add_command(self, _id: str, command: ICommand) -> None:
        self._buttons[_id] = command
...
if __name__ == "__main__":
    bedroom_light = Light('lampada', 'Quarto')
    bathroom_light = Light('lampada', 'Banheiro')
